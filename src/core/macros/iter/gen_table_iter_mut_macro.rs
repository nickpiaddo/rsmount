...
#[macro_export]
#[doc(hidden)]
macro_rules! gen_table_iter_mut {
    ($table_type:ident, $table_entry_type:ident) => {
        paste::paste! {
            ...
            #[doc = concat!("Iterator over mutable [`", stringify!($table_type), "`] entries.")]
            #[derive(Debug)]
            pub struct [<$table_type IterMut>]<'table> {
                table: &'table mut $table_type,
                /// Forward iterator.
            ...
            impl<'table> [<$table_type IterMut>]<'table> {
                #[doc = concat!("Creates a new `", stringify!($table_type), "IterMut`.")]
                #[allow(dead_code)]
                pub(crate) fn new(
                    table: &'table mut $table_type,
                ) -> Result<[<$table_type IterMut>]<'table>, [<$table_type IterError>]> {
            ...
            impl<'table> Iterator for [<$table_type IterMut>]<'table> {
                type Item = &'table mut $table_entry_type;
                    ...
                    match result {
                        0 => {
                                ...
                                || (self.fwd_cursor != self.bwd_cursor && ptr == self.bwd_cursor)
                            {
                                ...
                                self.fwd_cursor = ptr;
                                let entry = owning_mut_from_ptr!(self.table, $table_entry_type, ptr);

                                Some(entry)
                            }
                        }
                        1 => {
                            log::debug!(concat!(stringify!($table_type), "Iter::next reached the end of `", stringify!($table_type), "Iter`"));
                ...
                fn nth(&mut self, n: usize) -> Option<Self::Item> {
                    ...
                    for i in 0..n {
                        ...
                        match result {
                            ...
                            code => {
                                log::debug!(concat!(stringify!($table_type), "Iter::next failed to get the {:?}th table entry. libmount::mnt_table_next_fs returned error code: {:?}"), i, code);
            ...
            impl<'table> DoubleEndedIterator for [<$table_type IterMut>]<'table> {
                    ...
                    match result {
                        0 => {
                                ...
                                || (self.fwd_cursor != self.bwd_cursor && ptr == self.fwd_cursor)
                            {
                                ...
                                log::debug!(concat!(stringify!($table_type), "Iter::next_back got next table entry from the back"));
                ...
                fn nth_back(&mut self, n: usize) -> Option<Self::Item> {
                    ...
                    for i in 0..n {
                        ...
                        match result {
                            ...
                            code => {
                                log::debug!(concat!(stringify!($table_type), "Iter::nth_back failed to get the {:?}th table entry from the back. libmount::mnt_table_next_fs returned error code: {:?}"), i, code);
