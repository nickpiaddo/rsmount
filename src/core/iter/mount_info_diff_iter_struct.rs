...
// From this library
use crate::core::entries::MountInfoEntryDiff;
...
/// Iterate over [`MountInfoDiff`] entries.
#[derive(Debug)]
pub struct MountInfoDiffIter<'diff, 's, 'o> {
...
impl<'diff, 's, 'o> MountInfoDiffIter<'diff, 's, 'o> {
    #[doc(hidden)]
    /// Creates a new instance.
    #[allow(dead_code)]
    pub(crate) fn new(
        table_diff: &'diff MountInfoDiff<'s, 'o>,
    ) -> Result<MountInfoDiffIter<'diff, 's, 'o>, MountInfoDiffIterError> {
...
impl<'diff, 's, 'o> Iterator for MountInfoDiffIter<'diff, 's, 'o> {
    type Item = MountInfoEntryDiff<'diff, 's, 'o>;

    fn next(&mut self) -> Option<Self::Item> {
        log::debug!("MountInfoDiffIter::next getting next table changes");
        ...
        match result {
            0 => {
                ...
                {
                    log::debug!(
                        "MountInfoDiffIter::next forward and backward iterators have met in the middle"
                    );
                    ...
                    log::debug!("MountInfoDiffIter::next got next table changes");
                    self.first_fwd_cursor = source_entry_inner;
                    self.second_fwd_cursor = other_entry_inner;
                    let diff_entry = MountInfoEntryDiff::new(
                        self.table_diff,
                        source_entry_inner,
                        other_entry_inner,
                        flags,
            ...
            1 => {
                log::debug!("MountInfoDiffIter::next reached the end of `MountInfoDiffIter`");

                None
            }
            code => {
                let err_msg = "failed to get next changes".to_owned();
                log::debug!( "MountInfoDiffIter::next {err_msg}. libmount::mnt_tabdiff_next_change returned error code: {code:?}");
    ...
    fn nth(&mut self, n: usize) -> Option<Self::Item> {
        log::debug!("MountInfoDiffIter::nth getting {n:?}th table diff item");
                    ...
                    if self.have_iterators_met
                        || ((self.first_fwd_cursor != self.first_bwd_cursor)
                            ...
                            && (source_entry_inner == self.first_bwd_cursor
                                && other_entry_inner == self.second_bwd_cursor))
                    {
                        log::debug!( "MountInfoDiffIter::next forward and backward iterators have met in the middle");

                        self.have_iterators_met = true;

                        return None;
                    } else {
                        log::debug!("MountInfoDiffIter::nth got {i:?}th table diff item");
                        self.first_fwd_cursor = source_entry_inner;
                        self.second_fwd_cursor = other_entry_inner;
                    }
                }
                1 => {
                    log::debug!("MountInfoDiffIter::nth reached the end of `MountInfoDiffIter`");

                    return None;
                }
                code => {
                    let err_msg = format!("failed to get {:?}th table diff item", n);
                    log::debug!( "MountInfoDiffIter::nth {err_msg}. libmount::mnt_tabdiff_next_change returned error code: {code:?}");
...
impl<'diff, 's, 'o> DoubleEndedIterator for MountInfoDiffIter<'diff, 's, 'o> {
    fn next_back(&mut self) -> Option<Self::Item> {
        log::debug!("MountInfoDiffIter::next_back getting next table diff item from the back");
        ...
        match result {
            0 => {
                ...
                {
                    log::debug!( "MountInfoDiffIter::next_back forward and backward iterators have met in the middle");
                    ...
                    log::debug!(
                        "MountInfoDiffIter::next_back got next table changes iterating backward"
                    );
            ...
            1 => {
                log::debug!("MountInfoDiffIter::next_back reached the end of `MountInfoDiffIter`");

                None
            }
            code => {
                let err_msg = "failed to get next changes iterating backward".to_owned();
                log::debug!( "MountInfoDiffIter::next_back {err_msg}. libmount::mnt_tabdiff_next_change returned error code: {code:?}");
    ...
    fn nth_back(&mut self, n: usize) -> Option<Self::Item> {
        log::debug!("MountInfoDiffIter::nth_back getting {n:?}th table diff item from the back");
        ...
        for i in 0..n {
            ...
            match result {
                    ...
                    if self.have_iterators_met
                        || ((self.first_fwd_cursor != self.first_bwd_cursor)
                            ...
                            && (source_entry_inner == self.first_fwd_cursor
                                && other_entry_inner == self.second_fwd_cursor))
                    {
                        log::debug!( "MountInfoDiffIter::nth_back forward and backward iterators have met in the middle");
                        ...
                        log::debug!(
                            "MountInfoDiffIter::nth got {i:?}th table diff item from the back"
                        );
                        self.first_bwd_cursor = source_entry_inner;
                        self.second_bwd_cursor = other_entry_inner;
                    }
                ...
                1 => {
                    log::debug!(
                        "MountInfoDiffIter::nth_back reached the beginning of `MountInfoDiffIter`"
                    );
                ...
                code => {
                    ...
                    log::debug!( "MountInfoDiffIter::nth_back {err_msg}. libmount::mnt_tabdiff_next_change returned error code: {code:?}");
