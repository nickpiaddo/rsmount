...
// From this library
use crate::core::entries::SwapsEntryDiff;
...
/// Iterate over [`SwapsDiff`] entries.
#[derive(Debug)]
pub struct SwapsDiffIter<'diff, 's, 'o> {
...
impl<'diff, 's, 'o> SwapsDiffIter<'diff, 's, 'o> {
    #[doc(hidden)]
    /// Creates a new instance.
    #[allow(dead_code)]
    pub(crate) fn new(
        table_diff: &'diff SwapsDiff<'s, 'o>,
    ) -> Result<SwapsDiffIter<'diff, 's, 'o>, SwapsDiffIterError> {
...
impl<'diff, 's, 'o> Iterator for SwapsDiffIter<'diff, 's, 'o> {
    type Item = SwapsEntryDiff<'diff, 's, 'o>;

    fn next(&mut self) -> Option<Self::Item> {
        log::debug!("SwapsDiffIter::next getting next table changes");
        ...
        match result {
            0 => {
                ...
                {
                    log::debug!(
                        "SwapsDiffIter::next forward and backward iterators have met in the middle"
                    );
                    ...
                    log::debug!("SwapsDiffIter::next got next table changes");
                    self.first_fwd_cursor = source_entry_inner;
                    self.second_fwd_cursor = other_entry_inner;
                    let diff_entry = SwapsEntryDiff::new(
                        self.table_diff,
                        source_entry_inner,
                        other_entry_inner,
                        flags,
            ...
            1 => {
                log::debug!("SwapsDiffIter::next reached the end of `SwapsDiffIter`");

                None
            }
            code => {
                let err_msg = "failed to get next changes".to_owned();
                log::debug!( "SwapsDiffIter::next {err_msg}. libmount::mnt_tabdiff_next_change returned error code: {code:?}");
    ...
    fn nth(&mut self, n: usize) -> Option<Self::Item> {
        log::debug!("SwapsDiffIter::nth getting {n:?}th table diff item");
                    ...
                    if self.have_iterators_met
                        || ((self.first_fwd_cursor != self.first_bwd_cursor)
                            ...
                            && (source_entry_inner == self.first_bwd_cursor
                                && other_entry_inner == self.second_bwd_cursor))
                    {
                        log::debug!( "SwapsDiffIter::next forward and backward iterators have met in the middle");

                        self.have_iterators_met = true;

                        return None;
                    } else {
                        log::debug!("SwapsDiffIter::nth got {i:?}th table diff item");
                        self.first_fwd_cursor = source_entry_inner;
                        self.second_fwd_cursor = other_entry_inner;
                    }
                }
                1 => {
                    log::debug!("SwapsDiffIter::nth reached the end of `SwapsDiffIter`");

                    return None;
                }
                code => {
                    let err_msg = format!("failed to get {:?}th table diff item", n);
                    log::debug!( "SwapsDiffIter::nth {err_msg}. libmount::mnt_tabdiff_next_change returned error code: {code:?}");
...
impl<'diff, 's, 'o> DoubleEndedIterator for SwapsDiffIter<'diff, 's, 'o> {
    fn next_back(&mut self) -> Option<Self::Item> {
        log::debug!("SwapsDiffIter::next_back getting next table diff item from the back");
        ...
        match result {
            0 => {
                ...
                {
                    log::debug!( "SwapsDiffIter::next_back forward and backward iterators have met in the middle");
                    ...
                    log::debug!(
                        "SwapsDiffIter::next_back got next table changes iterating backward"
                    );
            ...
            1 => {
                log::debug!("SwapsDiffIter::next_back reached the end of `SwapsDiffIter`");

                None
            }
            code => {
                let err_msg = "failed to get next changes iterating backward".to_owned();
                log::debug!( "SwapsDiffIter::next_back {err_msg}. libmount::mnt_tabdiff_next_change returned error code: {code:?}");
    ...
    fn nth_back(&mut self, n: usize) -> Option<Self::Item> {
        log::debug!("SwapsDiffIter::nth_back getting {n:?}th table diff item from the back");
                    ...
                    if self.have_iterators_met
                        || ((self.first_fwd_cursor != self.first_bwd_cursor)
                            ...
                            && (source_entry_inner == self.first_fwd_cursor
                                && other_entry_inner == self.second_fwd_cursor))
                    {
                        log::debug!( "SwapsDiffIter::nth_back forward and backward iterators have met in the middle");

                        self.have_iterators_met = true;

                        return None;
                    } else {
                        log::debug!("SwapsDiffIter::nth got {i:?}th table diff item from the back");
                        self.first_bwd_cursor = source_entry_inner;
                        self.second_bwd_cursor = other_entry_inner;
                    }
                }
                1 => {
                    log::debug!("SwapsDiffIter::nth_back reached the beginning of `SwapsDiffIter`");

                    return None;
                }
                code => {
                    let err_msg = format!("failed to get {:?}th table diff item from the back", n);
                    log::debug!( "SwapsDiffIter::nth_back {err_msg}. libmount::mnt_tabdiff_next_change returned error code: {code:?}");
