...
impl MountSource {
    /// Returns `true` if this `MountSource` is a block device.
    pub fn is_block_device(&self) -> bool {
        matches!(self, Self::BlockDevice(_))
    }

    /// Returns `true` if this `MountSource` is a mount point.
    pub fn is_mount_point(&self) -> bool {
        matches!(self, Self::MountPoint(_))
    }

    /// Returns `true` if this `MountSource` is an NFS share address.
    pub fn is_nfs_share(&self) -> bool {
        matches!(self, Self::NFS(_))
    }

    /// Returns `true` if this `MountSource` is a SmbFs share address.
    pub fn is_samba_share(&self) -> bool {
        matches!(self, Self::SmbFs(_))
    }

    /// Returns `true` if this `MountSource` is an SSHFS address.
    pub fn is_sshfs_share(&self) -> bool {
        matches!(self, Self::SshFs(_))
    }

    /// Returns `true` if this `MountSource` is a tag (e.g `UUID=uuid`, `LABEL=label`,
    /// `PARTUUID=uuid`, etc.).
    pub fn is_tag(&self) -> bool {
        matches!(self, Self::Tag(_))
    }

    /// Returns `true` if this `MountSource` is a pseudo-filesystem.
    pub fn is_pseudo_fs(&self) -> bool {
        matches!(self, Self::PseudoFs(_))
    }

    /// Returns `true` if this `MountSource` is a `LABEL=label` tag.
    pub fn is_tag_label(&self) -> bool {
        matches!(self, Self::Tag(t) if t.is_label())
    }

    /// Returns `true` if this `MountSource` is a `PARTLABEL=label` tag.
    pub fn is_tag_partition_label(&self) -> bool {
        matches!(self, Self::Tag(t) if t.is_partition_label())
    }

    /// Returns `true` if this `MountSource` is a `UUID=uuid` tag.
    pub fn is_tag_uuid(&self) -> bool {
        matches!(self, Self::Tag(t) if t.is_uuid())
    }

    /// Returns `true` if this `MountSource` is a `PARTUUID=uuid` tag.
    pub fn is_tag_partition_uuid(&self) -> bool {
        matches!(self, Self::Tag(t) if t.is_partition_uuid())
    }

    /// Returns `true` if this `MountSource` is an `ID=id` tag.
    pub fn is_tag_id(&self) -> bool {
        matches!(self, Self::Tag(t) if t.is_id())
    }
}

impl AsRef<MountSource> for MountSource {
    #[inline]
    fn as_ref(&self) -> &MountSource {
        self
    }
}

impl TryFrom<&str> for MountSource {
...
impl TryFrom<String> for MountSource {
    type Error = ParserError;
...
impl From<BlockDevice> for MountSource {
    #[inline]
    fn from(device: BlockDevice) -> MountSource {
        MountSource::BlockDevice(device)
    }
}

impl From<MountPoint> for MountSource {
    #[inline]
    fn from(mount_point: MountPoint) -> MountSource {
        MountSource::MountPoint(mount_point)
    }
}

impl From<NFS> for MountSource {
    #[inline]
    fn from(share: NFS) -> MountSource {
        MountSource::NFS(share)
    }
}
...
impl From<SmbFs> for MountSource {
    ...
    fn from(share: SmbFs) -> MountSource {
        MountSource::SmbFs(share)
    }
}

impl From<SshFs> for MountSource {
    #[inline]
    fn from(share: SshFs) -> MountSource {
        MountSource::SshFs(share)
    }
}
...
impl From<Tag> for MountSource {
    ...
    fn from(share: Tag) -> MountSource {
        MountSource::Tag(share)
    }
}

impl From<Pseudo> for MountSource {
    #[inline]
    fn from(fs: Pseudo) -> MountSource {
        MountSource::PseudoFs(fs)
    }
}
...
mod tests {
    ...
    fn mount_source_does_not_parse_an_empty_string_as_a_block_device() {
        let source = "";
        let _ = MountSource::try_from(source).unwrap();
    }

    #[test]
    fn mount_source_parses_a_block_device() -> crate::Result<()> {
        let source = "/dev/vda";
        let actual: MountSource = source.parse()?;
    ...
    #[test]
    fn mount_source_parses_a_mount_point() -> crate::Result<()> {
        let source = "/boot";
        let actual: MountSource = source.parse()?;
    ...
    #[test]
    fn mount_source_parses_a_nfs_share_address_as_an_sshfs_share() -> crate::Result<()> {
    ...
    #[test]
    fn mount_source_parses_a_samba_share_address() -> crate::Result<()> {
    ...
    #[test]
    fn mount_source_parses_a_sshfs_share_address() -> crate::Result<()> {
    ...
    #[test]
    fn mount_source_parses_a_uuid_tag() -> crate::Result<()> {
    ...
    #[test]
    fn mount_source_parses_a_pseudo_fs() -> crate::Result<()> {
        let source = "none";
        let actual: MountSource = source.parse()?;
